...
def save_intermediates(
    intermediate_steps: List[Dict[str, torch.Tensor]],
    base_outdir: str,
    prompt_key: Any,
    vae: Any,
    image_processor: Any
    ...
    prompt_dir = os.path.join(base_outdir, f"prompt_{prompt_key}_steps")
    os.makedirs(prompt_dir, exist_ok=True)

    for step_data in intermediate_steps:
        t = step_data["timestep"]

        x0_pred = step_data["x0_pred"]       # clean prediction
        x_prev = step_data["x_prev"]         # x_{t-1}

        x0_image_tensor = vae.decode((x0_pred.to(vae.device) / vae.config.scaling_factor), return_dict=False)[0]


        x0_image = image_processor.postprocess(
            x0_image_tensor,
            output_type="pil",
            do_denormalize=[True]
        )[0]

        x0_image.save(os.path.join(prompt_dir, f"step_{t:04d}_x0_pred.png"))

        xprev_tensor = vae.decode((x_prev.to(vae.device) / vae.config.scaling_factor), return_dict=False)[0]


        xprev_image = image_processor.postprocess(
            xprev_tensor,
            output_type="pil",
            do_denormalize=[True]
        )[0]

        xprev_image.save(os.path.join(prompt_dir, f"step_{t:04d}_x_prev.png"))

    print(f"Saved {len(intermediate_steps)} intermediate steps for prompt {prompt_key} in {prompt_dir}")
...
def run_inference(args):
    ...
    try:
        ...
        print(f"Could not load '{model_path}' ({e}). Falling back to pretrained base.")
        pipe = StableDiffusionPipeline.from_pretrained(
            args.pretrained_model_name_or_path,
            safety_checker=None,
            revision=getattr(args, "revision", None),
    ...
    outdir="C://Users//aessalmi//Desktop//intermediate"
    os.makedirs(outdir, exist_ok=True)
    csv_path = os.path.join(outdir, "prompts.csv")
    ...
    print("\nGenerating images and intermediate steps...")
    ...
    with torch.inference_mode(), open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        for key, prompt in prompts.items():
            ...
            final_output, intermediate_steps = pipe(
                prompt,
                num_inference_steps=steps,
                generator=generator
            )

            final_img = final_output.images[0]
            final_filename = os.path.join(outdir, f"final_{key}.png")
